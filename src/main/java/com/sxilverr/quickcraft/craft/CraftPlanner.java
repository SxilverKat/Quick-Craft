...
public final class CraftPlanner {
    ...
    public static final class Blocker {
        ...
        public Blocker(ItemKey key, int missing, Reason reason) {
            this.key = key;
    ...
    public static final class Plan {
        private final CraftNode root;
        private final VirtualPool initial;
        private final VirtualPool working;
        private final EmcBank bank;
        private final Set<ItemKey> keys;
        private final int requested;
        private final int craftable;
        private final boolean truncated;
        private final List<Blocker> blockers;

        Plan(CraftNode root, VirtualPool initial, VirtualPool working, EmcBank bank, Set<ItemKey> keys,
        ...
        public CraftNode root() {
            return root;
        }

        public VirtualPool initial() {
            return initial;
        }

        public VirtualPool working() {
            return working;
        }
        ...
        public Map<ItemKey, Integer> supplied() {
            return bank == null ? Collections.<ItemKey, Integer>emptyMap() : bank.purchased();
        }
        ...
        public CraftPreview.Result toResult() {
            final ItemKey targetKey = ItemKey.of(root.output);
            ...
            Collections.sort(gained, new Comparator<CraftPreview.Gain>() {
                @Override
                public int compare(CraftPreview.Gain a, CraftPreview.Gain b) {
                    int ta = a.key().equals(targetKey) ? 0 : 1;
                    int tb = b.key().equals(targetKey) ? 0 : 1;
